...
ACTION_STD_DEV = np.array([0,0,0.05,0.05])  # noise applied to motion only
PROPORTIONAL_MOTION_NOISE = 0.5
...
def get_config(mode='train'):
  ...
  obstacle_factors = distribs.Product([
    ...
    distribs.Continuous('y', 0.4, 0.6)
  ])
  distractor_hue = distribs.Continuous('c0', 0.8, 0.85)
  # Generate the sprites to be used in this task, by combining Hue with the
  # other factors.
  sprite_factors = [
      distribs.Product((target_factors, shared_factors, c0)) for c0 in c0_clusters
  ]
  distractor_factors = distribs.Product([obstacle_factors, distractor_hue, shared_factors])
  ...
  sprite_gen_per_cluster += [distractor_sprite_gen]
  ...
  task = tasks.CollisionClustering(c0_clusters, distractor_factors, 
          obstacle_distance=0.15, collision_penalty=5,
          terminate_bonus=0., reward_range=10., sparse_reward=True)

  config = {
      'task': task,
      'action_space': common.noisy_action_space(ACTION_STD_DEV, PROPORTIONAL_MOTION_NOISE, c0_clusters),
      ...
      'metadata': {
          ...
          'mode': mode,
          'filter_distribs': c0_clusters, # DEMO ONLY
      },
  }
  return config
